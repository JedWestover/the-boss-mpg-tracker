...
function TrendChart({rows,title,field,color,format}){const [zoom,setZoom]=useState(1),[viewX,setViewX]=useState(0),[viewY,setViewY]=useState(0),[drag,setDrag]=useState(null);const values=rows.map(r=>Number(r[field])||0), max=Math.max(...values,1), min=Math.min(...values,0), range=max-min||1, width=640, height=220, pad={top:18,right:18,bottom:34,left:46},visibleWidth=width/zoom,visibleHeight=height/zoom;const points=rows.map((r,i)=>{const x=rows.length===1?width/2:pad.left+i*(width-pad.left-pad.right)/(rows.length-1);const y=pad.top+(max-(Number(r[field])||0))*(height-pad.top-pad.bottom)/range;return {...r,x,y,value:Number(r[field])||0};});const average=values.length?values.reduce((sum,value)=>sum+value,0)/values.length:0,averageY=pad.top+(max-average)*(height-pad.top-pad.bottom)/range,area=points.length>1?`${pad.left},${height-pad.bottom} ${points.map(p=>`${p.x},${p.y}`).join(' ')} ${points.at(-1).x},${height-pad.bottom}`:'';const setZoomLevel=nextZoom=>{const next=Math.max(1,Math.min(4,nextZoom));setZoom(next);setViewX(Math.min(width-visibleWidth,Math.max(0,(width-width/next)/2)));setViewY(Math.min(height-visibleHeight,Math.max(0,(height-height/next)/2)));};const resetView=()=>{setZoom(1);setViewX(0);setViewY(0);};const pan=e=>{if(!drag)return;const rect=e.currentTarget.getBoundingClientRect(),scaleX=visibleWidth/rect.width,scaleY=visibleHeight/rect.height;setViewX(Math.max(0,Math.min(width-visibleWidth,drag.viewX-(e.clientX-drag.x)*scaleX)));setViewY(Math.max(0,Math.min(height-visibleHeight,drag.viewY-(e.clientY-drag.y)*scaleY)));};return <article className="chart"><div className="chartHead"><div><h3>{title}</h3><span>{rows.length?'Per fill-up':'No entries yet'}</span></div>{rows.length&&<div style={{display:'flex',gap:'6px'}}><button className="button secondary" style={{padding:'5px 9px'}} type="button" onClick={()=>setZoomLevel(zoom/1.25)} aria-label="Zoom out">−</button><button className="button secondary" style={{padding:'5px 9px'}} type="button" onClick={()=>setZoomLevel(zoom*1.25)} aria-label="Zoom in">+</button><button className="button secondary" style={{padding:'5px 9px'}} type="button" onClick={resetView}>Reset</button></div>}</div>{rows.length?<svg viewBox={`${viewX} ${viewY} ${visibleWidth} ${visibleHeight}`} role="img" aria-label={title} style={{cursor:zoom>1?'grab':'default',touchAction:'none'}} onPointerDown={e=>{if(zoom>1){e.currentTarget.setPointerCapture(e.pointerId);setDrag({x:e.clientX,y:e.clientY,viewX,viewY});}}} onPointerMove={pan} onPointerUp={()=>setDrag(null)} onPointerCancel={()=>setDrag(null)}><line className="axis" x1={pad.left} x2={pad.left} y1={pad.top} y2={height-pad.bottom}/><line className="axis" x1={pad.left} x2={width-pad.right} y1={height-pad.bottom} y2={height-pad.bottom}/><polygon className="chartArea" points={area} fill={color}/><line className="chartTrend" x1={pad.left} x2={width-pad.right} y1={averageY} y2={averageY} fill="none" stroke="#eaf0f8" strokeWidth="2" strokeDasharray="6 5" opacity=".7"><title>{`Average: ${format(average)}`}</title></line><polyline className="chartLine" points={points.map(p=>`${p.x},${p.y}`).join(' ')} stroke={color}/>{points.map(p=><circle key={p.id} cx={p.x} cy={p.y} r="4" fill={color}><title>{`${p.date}: ${format(p.value)}`}</title></circle>)}<text className="chartLabel" x={pad.left} y={height-10}>{rows[0].date}</text><text className="chartLabel" textAnchor="end" x={width-pad.right} y={height-10}>{rows.at(-1).date}</text><text className="chartValue" x={pad.left-8} y={pad.top+4} textAnchor="end">{format(max)}</text><text className="chartValue" x={pad.left-8} y={height-pad.bottom+4} textAnchor="end">{format(min)}</text></svg>:<div className="chartEmpty">Add a fill-up to start tracking this trend.</div>}</article>}
...
function App(){
 ...
 const signOutOneDrive=async()=>{if(!msal||!account)return;try{await msal.logoutPopup({account,mainWindowRedirectUri:window.location.origin});setAccount(null);setSyncStatus('OneDrive signed out; using local storage');}catch(error){setSyncStatus(`OneDrive sign out failed: ${error?.errorCode||error?.message||'Unknown error'}`);}};
 const switchOneDriveUser=async()=>{if(!msal||!account)return;try{setSyncStatus('Signing out…');await msal.logoutPopup({account,mainWindowRedirectUri:window.location.origin});setAccount(null);setSyncStatus('Choose a OneDrive account');const result=await msal.loginPopup(loginRequest);const signedIn=result.account;setAccount(signedIn);try{const remote=await cloudRequest(signedIn);if(remote?.fuel&&remote?.def){setData(remote);localStorage.setItem(KEY,JSON.stringify(remote));setSyncStatus('Synced from OneDrive');}else{setSyncStatus('Connected to OneDrive');}}catch(error){if(error.message!=='missing')throw error;const emptyData={fuel:[],def:[]};await cloudRequest(signedIn,'PUT',emptyData);setData(emptyData);localStorage.setItem(KEY,JSON.stringify(emptyData));setSyncStatus('Created new OneDrive profile');}}catch(error){setSyncStatus(`OneDrive switch failed: ${error?.errorCode||error?.message||'Unknown error'}`);}};
 ...
 <footer>{account?'Data is saved locally and synced to OneDrive when connected.':'Data is saved in this browser. Connect OneDrive for automatic sync.'} Export a backup for an extra copy.</footer></main>}
