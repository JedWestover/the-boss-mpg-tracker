...
const localRedirectUri = "http://localhost:5173/auth.html";
...
const msal = microsoftClientId
  ? new PublicClientApplication({
      auth: {
        ...
        redirectUri,
        postLogoutRedirectUri: "https://the-boss-mpg-tracker.vercel.app",
      },
      cache: {
...
async function cloudRequest(account, method = "GET", body) {
  const token = await graphToken(account);
  const response = await fetch(
    `https://graph.microsoft.com/v1.0/me/drive/root:/${CLOUD_FILE}:/content`,
    {
      method,
      headers: {
  ...
  if (!response.ok)
    throw Error(response.status === 404 ? "missing" : "request");
  ...
  return [...rows]
    ...
    .map((r, i, all) => {
      ...
      return {
        ...r,
        distance,
        efficiency: r.gallons ? distance / r.gallons : 0,
        unitCost: r.gallons ? r.cost / r.gallons : 0,
        costMile: distance ? r.cost / distance : 0,
      };
    });
}
function Summary({ rows, type }) {
  const totalMiles = rows.reduce((s, r) => s + r.distance, 0),
    ...
    average = valid.length
      ...
      : 0,
    best = valid.length ? Math.max(...valid.map((r) => r.efficiency)) : 0,
    worst = valid.length ? Math.min(...valid.map((r) => r.efficiency)) : 0;
  return (
    <div className={`cards ${type === "fuel" ? "dashboard" : ""}`}>
      {type === "fuel" ? (
        <>
          <article>
        ...
        <>
          <article>
            ...
            <b>{num(totalMiles, 0)}</b>
          </article>
          <article>
            <span>Miles / DEF gal</span>
            <b>{num(gallons ? totalMiles / gallons : 0)}</b>
          </article>
          <article>
...
function TrendChart({ rows, title, field, color, format }) {
  ...
  const values = rows.map((r) => Number(r[field]) || 0),
    ...
    width = 640,
    height = 220,
    pad = { top: 18, right: 18, bottom: 34, left: 46 },
  ...
  const points = rows.map((r, i) => {
    const x =
      rows.length === 1
        ? width / 2
        : pad.left + (i * (width - pad.left - pad.right)) / (rows.length - 1);
    const y =
      pad.top +
      ((max - (Number(r[field]) || 0)) * (height - pad.top - pad.bottom)) /
        range;
    return { ...r, x, y, value: Number(r[field]) || 0 };
  });
  const average = values.length
      ? values.reduce((sum, value) => sum + value, 0) / values.length
      : 0,
    averageY =
      pad.top + ((max - average) * (height - pad.top - pad.bottom)) / range,
    area =
      points.length > 1
      ...
      {rows.length ? (
        ...
        >
          ...
          <line
            ...
            y2={height - pad.bottom}
          />
          <polygon className="chartArea" points={area} fill={color} />
          <line
            className="chartTrend"
            x1={pad.left}
            x2={width - pad.right}
            y1={averageY}
            y2={averageY}
            ...
            opacity=".7"
          >
            <title>{`Average: ${format(average)}`}</title>
          </line>
          <polyline
            className="chartLine"
          ...
          <text className="chartLabel" x={pad.left} y={height - 10}>
            {rows[0].date}
          </text>
          <text
            className="chartLabel"
        ...
        <div className="chartEmpty">
          Add a fill-up to start tracking this trend.
        </div>
      )}
    </article>
  );
...
function App() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const cached = msal.getAllAccounts()[0];
        if (!cached) return;
        setAccount(cached);
        const remote = await cloudRequest(cached);
        if (active && remote?.fuel && remote?.def) {
          setData(remote);
          localStorage.setItem(KEY, JSON.stringify(remote));
          setSyncStatus("Synced from OneDrive");
        }
      } catch {
        if (active) setSyncStatus("OneDrive sync unavailable");
      }
    })();
    return () => {
      active = false;
  ...
  const save = (n) => {
    ...
    if (account) {
      ...
      cloudRequest(account, "PUT", n)
        .then(() => setSyncStatus("Synced to OneDrive"))
        .catch(() => setSyncStatus("Saved locally; OneDrive sync failed"));
    }
  };
  const signOutOneDrive = async () => {
    ...
    try {
      ...
      setAccount(null);
      setSyncStatus("OneDrive signed out; using local storage");
    } catch (error) {
      setSyncStatus(
        `OneDrive sign out failed: ${error?.errorCode || error?.message || "Unknown error"}`,
      );
    }
  };
  const switchOneDriveUser = async () => {
    if (!msal || !account) return;
    try {
      ...
      setAccount(null);
      setSyncStatus("Choose a OneDrive account");
      const result = await msal.loginPopup(loginRequest);
      const signedIn = result.account;
      setAccount(signedIn);
      try {
        const remote = await cloudRequest(signedIn);
        if (remote?.fuel && remote?.def) {
          setData(remote);
          localStorage.setItem(KEY, JSON.stringify(remote));
          setSyncStatus("Synced from OneDrive");
        } else {
          setSyncStatus("Connected to OneDrive");
        }
      } catch (error) {
        if (error.message !== "missing") throw error;
        const emptyData = { fuel: [], def: [] };
  ...
  const exportFile = () => {
    ...
    for (const t of ["fuel", "def"]) {
      const out = calc(data[t]).map((r) => ({
        Type: t === "fuel" ? "Fuel" : "DEF",
        Date: r.date,
        Odometer: r.odometer,
        "Miles since prior fill": r.distance,
        Gallons: r.gallons,
        Cost: r.cost,
        "Cost/Gallon": r.unitCost,
        "Miles/Gallon": r.efficiency,
        "Cost/Mile": r.costMile,
      }));
    ...
    XLSX.writeFile(wb, "The-Boss-MPG-Backup.xlsx");
  };
  const importFile = async (e) => {
    ...
    try {
      ...
      const imported = { fuel: [], def: [] };
      const parse = (sheet, type) => {
        const a = XLSX.utils.sheet_to_json(sheet, { defval: "" });
        a.forEach((r, i) => {
          const date = r.Date || r.DATE;
          const odo = +(r.Odometer || r["EndODO"] || r["END ODO"]);
          const gallons = +(r.Gallons || r["TOTAL GALLONS"]);
          const cost = +(r.Cost || r["TOTAL FUEL COST"] || r["TOTAL DEF COST"]);
          const distance = +(
            r["Miles since prior fill"] ||
            r["TRIP METER"] ||
            r["Trip miles"]
          );
          if (date && gallons) {
      ...
      wb.SheetNames.forEach((n) =>
        parse(wb.Sheets[n], n.toLowerCase().includes("def") ? "def" : "fuel"),
      );
      if (!imported.fuel.length && !imported.def.length) throw Error();
      save({
        fuel: imported.fuel.length ? imported.fuel : data.fuel,
        def: imported.def.length ? imported.def : data.def,
      });
      setMsg("Spreadsheet imported.");
    } catch {
      setMsg(
        "Could not read that layout. Use the exported backup format or the original tracker workbook.",
      );
    }
    e.target.value = "";
  };
  return (
    <main>
      <header>
        <div>
          <h1>MPG Tracker</h1>
          <p>Fuel and DEF fill-up history, all in one place.</p>
        </div>
        <div className="actions">
          {account ? (
            <>
              <button className="button secondary" onClick={switchOneDriveUser}>
                Switch user
              </button>
              <button className="button secondary" onClick={signOutOneDrive}>
                Sign out
              </button>
            </>
          ) : (
            <button
              ...
              disabled={!msal}
            >
              {msal ? "Connect OneDrive" : "Set Microsoft client ID"}
            </button>
          )}
        ...
        <form onSubmit={add}>
          ...
          <label>
            ...
            <input
              ...
              value={form.odometer}
              onChange={(e) =>
                setForm({ ...form, odometer: e.target.value.replace(/,/g, "") })
              }
            />
          </label>
          <label>
      ...
      <section className="panel tablePanel">
        <div className="sectionHead">
          ...
          <div>
            ...
            <button
              className="button secondary historyToggle"
              type="button"
            ...
            <table>
              ...
              <tbody>
                {[...rows].reverse().map((r) => (
                  <tr key={r.id}>
                    <td>{r.date}</td>
                    <td>{num(r.odometer, 0)}</td>
                    <td>{num(r.distance, 0)}</td>
                    <td>{num(r.gallons)}</td>
                    <td>{money.format(r.cost)}</td>
      ...
      <footer>
        {account
          ? "Data is saved locally and synced to OneDrive when connected."
